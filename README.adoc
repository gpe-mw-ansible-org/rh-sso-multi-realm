...
$ echo "export ocp_sso_admin_username=sso0" >> ~/.bashrc
$ echo "export rhsso_project=rhsso-\$ocp_sso_admin_username" >> ~/.bashrc
...
The OCP namespace for the RH-SSO multi-realm application will be owned by the following user: {{ocp_sso_admin_username}}.
{{ocp_sso_admin_username}} will be assigned a clusterquota so as to manage limits and requests assigned to 3scale
...
$ ansible-playbook -i localhost, -c local /tmp/rh_sso_multi_realm.yml \
                    ...
                    -e"admin_username=$ocp_sso_admin_username" \
...
+
-----
echo "export rhsso_url=https://`oc get route sso -n rhsso-$ocp_sso_admin_username --template "{{.spec.host}}" -n rhsso-$ocp_sso_admin_username`" >> ~/.bashrc
