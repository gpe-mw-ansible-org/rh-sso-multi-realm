...
+
-----
echo "export rhsso_url=`oc get route sso -n rhsso-$ocp_sso_admin_id --template "{{.spec.host}}" -n rhsso-$ocp_sso_admin_id`" >> ~/.bashrc
...
+
-----
$ curl -v -X GET "https://$rhsso_url/auth/realms/master/.well-known/openid-configuration" | python -m json.tool
...
+
-----
$ echo -en "\nhttps://$rhsso_url/auth/admin/master/console\n\n"
-----
...
. *realmId:*  kie-realm
. *Login URL:* https://$rhsso_url/auth/admin/kie-realm/console
. *Admin User Id:* admin
...
. *realmId:*  ocp-realm
. *Login URL:* https://$rhsso_url/auth/admin/ocp-realm/console
...
====    Configuring OpenID Identity Provider

After provisioning of the `ocp-realm` in RH-SSO, there are some additional steps to execute in the master node of OCP.

. View the contents of the file {{new_app_output_dir}}/`ocp-realm-suggestion.txt`

. Scroll to the section *In the master node of your OCP deployment, add the following*.
...
. After the Master API service of the OCP cluster has been restarted, follow the remaining instructions in `ocp-realm-suggestion.txt` to retrieve the access token.

[NOTE]
Should you encounter an error page indicating an incorrect redirect URI, logon to the RH-SSO Admin Console, navigate to the `ocp-realm` and change the value of the *Valid Direct URIs* setting in the client called `ocp-client` to the correct value.
For instance, the redirect URI should be in such a format:

image::images/valid-redirect-url.png[]
...
+
-----
$ echo -en "\nhttps://$rhsso_url/auth/admin/$rhsso_realm/console\n\n"
-----
...
+
-----
$ curl -X GET "https://$rhsso_url/auth/realms/$rhsso_realm/.well-known/openid-configuration" | python -m json.tool
-----
...
+
-----
$ curl -X GET "https://$rhsso_url/auth/realms/$rhsso_realm/.well-known/openid-configuration" | jq -r '.grant_types_supported'
-----
+
.Sample Output
-----
...
+
-----
$ retrieve_token_url="https://$rhsso_url/auth/realms/$rhsso_realm/protocol/openid-connect/token"
...
+
-----
$ retrieve_key_url="https://$rhsso_url/auth/admin/realms/$rhsso_realm/keys"
-----
...
$ echo "
{
  \"realm\": \"$rhsso_realm\",
  \"bearer-only\": \"true\",
  \"auth-server-url\": \"http://$rhsso_url/auth\",
