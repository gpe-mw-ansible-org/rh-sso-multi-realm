...
. *SSO user:* Configurable number of users associated with this realm are created as per the variables: `start_user` and `end_user`
. *OCP related roles:*
+
...
==== SMTP Providers
It's likely that you will want your RH-SSO realms to have the ability to send emails.
...
A couple of SMTP providers with _Free Plans_ are listed below:
