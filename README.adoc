...
. This ansible workload assumes that your pre-existing OCP cluster is configured with a certificate signed by a legitimate Certificate Authority (such as LetsEncrypt).
...
This section lists ansible commands to create and delete SSO realms

=== Generic Realms

A configurable number of SSO realms will be created based on the value of the following variables: `start_realm` and `end_realm`..

If the value of `end_realm` is less than 1, then generic tenants will not be created.
...
If when creating realms the variable _loadOCPRealm_ is set to true, then OCP will configured to link:https://access.redhat.com/documentation/en-us/red_hat_jboss_middleware_for_openshift/3/html/red_hat_single_sign-on_for_openshift/tutorials#OSE-SSO-AUTH-TUTE[use RH-SSO for Authentication] .
...
. *OCP related roles:*
+
None.  Roles must be assigned by a cluster admin as per the instructions in : {{new_app_output_dir}}/ocp-realm-suggestion.txt
...
#### UPDATE ME !!!!
START_REALM=1               # first realm to be initialized will be:  realm$START_REALM
END_REALM=1                 # last realm to be initialized will be :  realm$END_REALM ; if value is less than 1, then generic realms will not be created
...
end_user=1                  # last user to be created in ocp-realm will be:  user$end_user
