...
$ echo "export ocp_sso_admin_prefix=sso0" >> ~/.bashrc
$ echo "export rhsso_project=rhsso-\$ocp_sso_admin_prefix" >> ~/.bashrc
...
+
-----
$ ansible-galaxy install jbride.rh_sso_multi_realm
-----
...
- hosts: all
  ...
  vars_files:
  roles:
    - jbride.rh_sso_multi_realm
" > /tmp/rh_sso_multi_realm.yml
-----

=== Provision RH-SSO

The OCP namespace for the RH-SSO multi-realm application will be owned by the following user: {{ocp_sso_admin_prefix}}.
A user called {{ocp_sso_admin_prefix}} will be assigned a clusterquota so as to manage limits and requests assigned to 3scale
...
$ ansible-playbook -i localhost, -c local /tmp/rh_sso_multi_realm.yml \
                    ...
                    -e"admin_username=$ocp_sso_admin_prefix" \
...
+
-----
echo "export rhsso_url=https://`oc get route sso -n rhsso-$ocp_sso_admin_prefix --template "{{.spec.host}}" -n rhsso-$ocp_sso_admin_prefix`" >> ~/.bashrc
...
This section lists ansible commands to create and delete a configurable number of SSO realms in a previously provisioned RH-SSO installation.
...
#### UPDATE ME !!!!
START_REALM=1               : name of first realm to be initialized will be:  user$START_REALM
END_REALM=1                 : name of last realm to be initialized will be :  user$END_REALM
